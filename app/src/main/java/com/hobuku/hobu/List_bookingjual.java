...
public class List_bookingjual extends ListActivity {
String ip="";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(List_bookingjual.this);
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				...
				Intent i = new Intent(getApplicationContext(), Detail_beliarsip.class);//Detail_belibooking
				i.putExtra("pk", pk);
	...
	class load extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(List_bookingjual.this);
		...
		protected String doInBackground(String... args) {
			...
			JSONObject json = jParser.makeHttpRequest(ip+"beli/booking.php", "GET", params);
			...
			try {
				...
				if (sukses == 1) {
					...
					for (int i = 0; i < myJSON.length(); i++) {
						...
						String kode_jual = c.getString("jenis_burung").toUpperCase()+" /Kode Jual:"+c.getString(TAG_kode_jual);
						String tanggal_beli = c.getString(TAG_tanggal_beli)+" - "+c.getString("jam_beli")+" WIB\nPembeli: "+c.getString("nama_anggota")+" ("+c.getString("status_beli")+")";
		...
		protected void onPostExecute(String file_url) {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					ListAdapter adapter = new SimpleAdapter(List_bookingjual.this, arrayList,R.layout.desain_list, new String[] { TAG_kode_beli,TAG_kode_jual, TAG_tanggal_beli,},new int[] { R.id.kode_k, R.id.txtNamalkp ,R.id.txtDeskripsilkp});
